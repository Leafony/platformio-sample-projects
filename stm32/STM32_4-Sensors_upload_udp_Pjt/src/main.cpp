...
//     Platform     : STM32
//     Processor    : STM32 MCU
//     Application  : 4-Sensors_upload_udp
//
//     Leaf configuration
//       (1) AI01 4-Sensors
//       (2) AP03 STM32 MCU
...
//
//      Rev.00 2022/8/28  First release
//=====================================================================
/*
  Web client
...
const int HOUR_START = 7;  // 24 hour
const int HOUR_STOP = 6; // 24 hour
...
void loop() {
  ...
  //---------------------------
  Serial.println("Starting Arduino udp client  ======================");
  ...
  if (lpwaAccess.begin() != LPWA_READY) {
    ...
    Serial.println("Restart after 5 sec");
    delay(10);
    LowPower.deepSleep(5000); // msec
    return;
  }
  ...
  if (active == true) {  //............... if active time
    ...
    Serial.println(String(dataTemp) + "[degC], " + String(dataHumid) + "[%]");
    ...
    Serial.println("Illuminance: " + String(dataIllum));
#endif
    ...
    Udp.write(sendBinary, sizeof(sendBinary)/sizeof(sendBinary[0]));
    Udp.endPacket();
    delay(1000);
#endif
